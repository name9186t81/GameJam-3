...
namespace Effects
{
	public sealed class TilemapCycle : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			var repeatDelta = 1 - Mathf.Abs(1 - (delta * 2));
			...
			Color mapColor = _tileMapColor.Evaluate(repeatDelta);
			Color shadowColor = _shadowColor.Evaluate(repeatDelta);
